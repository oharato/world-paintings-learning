...
- [x] 9.1. Metropolitan Museum of Art API の調査とテスト
    - [x] API仕様の確認
    - [x] データ取得スクリプトの作成 (fetch-paintings.mts)
- [x] 9.2. 絵画データの準備
    - [x] paintings.ja.json と paintings.en.json の作成（14作品）
    - [x] プレースホルダーSVG画像の生成
    - [x] 有名作家の作品収集（ゴッホ、モネ、ルノワール、ピカソなど）
- [x] 9.3. データストアの実装
    - [x] paintings.ts ストアの作成
    - [x] Painting インターフェースの定義
    - [x] paintingQuiz.ts ストアの作成（デュアル選択対応）
- [x] 9.4. クイズ画面の実装
    - [x] PaintingQuizSetup コンポーネント（設定画面）
    - [x] PaintingQuizPlay コンポーネント（クイズ画面）
        - [x] 絵画の表示
        - [x] 2列グリッドレイアウト（左：作者、右：タイトル）
        - [x] デュアル選択UI
        - [x] 両方選択必須の回答ボタン
    - [x] PaintingQuizResult コンポーネント（結果画面）
        - [x] スコア表示（完全正解：1000pt、部分正解：500pt）
        - [x] 回答詳細一覧（画像付き）
        - [x] 正誤の視覚的フィードバック
- [x] 9.5. ルーティングとナビゲーション
    - [x] /painting-quiz ルートの追加
    - [x] /painting-quiz/play ルートの追加
    - [x] /painting-quiz/result ルートの追加
    - [x] ホーム画面に名画クイズリンクを追加
- [x] 9.6. ビルドとテスト
    - [x] TypeScriptコンパイル確認
    - [x] Lint/Format の実行
