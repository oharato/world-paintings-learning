...
2.  **国データの生成と画像ダウンロード**
    ...
    - 国旗の由来（"Flag of [Country]" ページから取得、英語版では正規化処理を適用）
    ...
    **国旗の由来取得について**:
    英語版の国旗情報取得では、以下の改善が行われています：
    - 国名の正規化: "the"、"Republic of"、"Democratic Republic of" などの接頭辞を除去して Wikipedia ページを検索
    - 複数のページ名バリエーションを試行: 見つからない場合は別の形式で再試行
    - HTMLからのクリーンアップ: リンクや注釈（[1], [2], [citation needed]など）を自動除去
